...
init()
{
    // Enable bot killstreaks by setting the Dvar to 0
    setDvar("disable_bot_killstreaks", 0);

    // Override the existing h2_setCustomKillstreaks function to use our custom logic
    level.h2_setCustomKillstreaks = ::custom_h2_setCustomKillstreaks;
	h2_set_bot_ks = ::custom_h2_set_bot_ks;

    // Re-run the killstreak setup for all bots
    foreach (player in level.players)
    {
        if (isBot(player))
        {
            player thread h2_setCustomKillstreaks(); // This should now call the custom_h2_set_bot_ks
        }
    }

    // Optionally print out a message for debugging
    println("Bot killstreaks are now enabled with custom logic.");
}
...
disableBotKillstreaks()
{
    ...
    foreach (player in level.players)
    {
        if (isBot(player))
        {
            player.customKillstreaks = undefined; // Clear any existing killstreaks
        }
    }

    println("Bot killstreaks are now disabled.");
}


custom_h2_setCustomKillstreaks()
{
	...
	if( isBot( self ) )
	{
		if (!getDvarInt("disable_bot_killstreaks"))
		{
			self.customKillstreaks = [];
            self thread custom_h2_set_bot_ks();  // Call your custom function
		}
	}
	else
	{
...
custom_h2_set_bot_ks()
{
	...
	case 5:
		self.customKillstreaks[0] = "harrier_airstrike_mp";
		self.customKillstreaks[1] = "ac130_mp";
